...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////Users/tingyang/Desktop/CourseHub/courseHub.db'
...
def course(course_id):
    context = {
        'post': Post.query.filter(Post.course_id == course_id).order_by(Post.post_time.desc()).all()
    }
...
def usercenter(username, tag):
    ...
    page = request.args.get(get_page_parameter(), type=int, default=1)
    pagination1 = user.post.paginate(page=page, per_page=10, error_out=False)
    pagination2 = user.comment.paginate(page=page, per_page=10, error_out=False)
    comments = pagination2.items
    posts = pagination1.items
    if tag == 'post':
        return render_template('usercenter1.html', user=user, pagination1=pagination1, posts=posts)
    else:
        return render_template('usercenter2.html', user=user, pagination2=pagination2, comments=comments)
